...
def answer_modify(request,answer_id):
    ...
    if request.user != answer.author:
        ...
        return redirect('pybo:detali', question_id=answer.question_id)
    if request.method == 'POST':
...
def answer_vote(request,answer_id):
    answer = get_object_or_404(Answer, pk=answer_id)
    if request.user == answer.author:
        return JsonResponse({'message':'자신의 글은 추천할 수 없습니다.'},status=405)
    if request.user in answer.voter.all():
        return JsonResponse({'message':'이미 추천을 했습니다.'},status=405)
    answer.voter.add(request.user)
    return JsonResponse({'message':answer.voter.count()})
